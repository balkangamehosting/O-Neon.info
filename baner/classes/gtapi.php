...
class GTApi {
	...
	public function GraficonInBanner($bannercolor = "cs") {
		...
		if($this->apilink->apiError != 1) {
	        ...
	        $img = imagecreatefrompng('http://'.$_SERVER['HTTP_HOST'].'/grafik/index.php?ip='.$this->ip.'&port='.$this->port.'&baner=true');
	         
	        $black = imagecolorallocate($img, 255,255, 255);    
	         
	        imagettftext($bg, 7, 0, 33, 83, imagecolorallocate($bg,255,255,255), "graficon/fonts/slova.otf", $this->ip.":".$this->port);
	        
	        if($this->apilink->status == 1) {
	       		imagettftext($bg, 7, 0, 228, 83, imagecolorallocate($bg,86,217,123), "graficon/fonts/slova.otf", "Online!");
	        } else {
	        	imagettftext($bg, 7, 0, 228, 83, imagecolorallocate($bg,255,0,0), "graficon/fonts/slova.otf", "Offline!");
	        }

	        imagettftext($bg, 7, 0, 33, 123, imagecolorallocate($bg,255,255,255), "graficon/fonts/slova.otf", $this->apilink->map);
	        imagettftext($bg, 7, 0, 153, 123, imagecolorallocate($bg,255,255,255), "graficon/fonts/slova.otf", $this->apilink->players . "/" . $this->apilink->playersmax);
	        imagettftext($bg, 7, 0, 253, 123, imagecolorallocate($bg,255,255,255), "graficon/fonts/slova.otf", $this->apilink->rank);
	        imagettftext($bg, 7, 0, 33, 47, imagecolorallocate($bg,255,255,255), "graficon/fonts/slova.otf", $this->apilink->name);
	     ...
	     //   imagettftext($bg, 7, 0, 7, 111, imagecolorallocate($bg,255,255,255), "graficon/fonts/slova.otf", $this->apilink->gamename);
	      ...
	      //  imagettftext($bg, 7, 0, 5, 111, imagecolorallocate($bg,255,255,255), "slova.otf", $this->apilink->gameshort);
	        ...
	        imagecopymerge($bg, $img, 337, 15, 0, 0, 160, 77, 100);

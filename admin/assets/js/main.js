...
function izbrisi_vest(id){
	var dataString = 'task=delvest&id=' + id;
...
function edit_vest(id, naslov, poruka, l_slika) {
	$("#naslovs").val(naslov);
	$("#texts").val(poruka);
	$("#l_slika").val(l_slika);
	$("input.id_sl").val(id);
...
function mod(id, ime, opis, putanja, igra, cena, mapa, sakriven, komanda) {
	$(".mod_id").val(id);
	$(".igra_id").val(igra);
	$("#modime").val(ime);
	$("#modopis").val(opis);

...
$tiket = mysql_query("SELECT * FROM `tiketi` WHERE `id` = '$tiket_id'");
...
$tiket_odgovori = mysql_query("SELECT * FROM `tiketi_odgovori` WHERE `tiket_id` = '$tiket_id' ORDER BY `id`");
...
if($tiket_inf['naslov'] == "Billing: <z>Nova uplata</z>" or $tiket_inf['naslov'] == "Billing: Nova uplata - Leglo" or $tiket_inf['naslov'] == "Billing: Nova uplata - Nije leglo"){
	$server = "Nova uplata";
}
else
{
	$srv = query_fetch_assoc("SELECT * FROM `serveri` WHERE `id` = '".$tiket_inf['server_id']."'");
	$server = '<a href="srv-pocetna.php?id='.$srv['id'].'">'.$srv["name"].'</a>';
	unset($srv);
}
...
<?php 
					...
					<?php	}	?>
							...
							<div class="btn btn-primary" onclick="dodajKomentar_Tiket()" style="margin: 0 10px 10px 10px;">Pošalji</div>						
						</ul>
					</div>
	    ...
	    <div class="span4">
			...
			<?php if($tiket_inf['status'] == "3") { ?>
					<form action="process.php" method="POST">
						<input type="hidden" name="task" value="odkljucaj_tiket" />
			...
			<?php } else { ?>
		      ...
		      <?php } ?>				
					...
					<a id="t_prosl" href="javascript:;" class="btn btn-large btn-info btn-support-ask">Prosledi tiket</a>	
					
					<a id="t_ban" href="javascript:;" class="btn btn-large btn-inverse btn-support-contact">Banuj klijenta</a>
<?php				if($tiket_inf['naslov'] == "Billing: <z>Nova uplata</z>")
					{	
?>
					<br />
					<form action="process.php" method="POST">
						<input type="hidden" name="task" value="uplata" />
						<input type="hidden" name="statust" value="2" />
						<input type="hidden" name="tiketid" value="<?php echo $tiket_id; ?>" />
						<input type="hidden" name="userid" value="<?php echo $tiket_inf['user_id']; ?>" />
						<button style="width: 100%;" type="submit" name="status" class="btn btn-large btn-warning btn-support-contact">Prihvati i stavi na proveru</button>
					</form>
					<form action="process.php" method="POST">
						<input type="hidden" name="task" value="uplata" />
						<input type="hidden" name="statust" value="0" />
						<input type="hidden" name="tiketid" value="<?php echo $tiket_id; ?>" />	
						<input type="hidden" name="userid" value="<?php echo $tiket_inf['user_id']; ?>" />						
						<button style="width: 100%;" type="submit" name="status" class="btn btn-large btn-danger btn-support-contact">Odbij uplatu</button>
					</form>						
<?php	
					}	
					else if($tiket_inf['naslov'] == "Billing: Nova uplata - Ceka proveru")
					{
?>		
					<br />
					<form action="process.php" method="POST">
						<input type="hidden" name="task" value="uplata" />
						<input type="hidden" name="statust" value="1" />
						<input type="hidden" name="tiketid" value="<?php echo $tiket_id; ?>" />
						<input type="hidden" name="userid" value="<?php echo $tiket_inf['user_id']; ?>" />
						<button style="width: 100%;" type="submit" name="status" class="btn btn-large btn-warning btn-support-contact">Prihvati uplatu</button>
					</form>
					<form action="process.php" method="POST">
						<input type="hidden" name="task" value="uplata" />
						<input type="hidden" name="statust" value="0" />
...
?>					
			...
			<div class="widget stacked widget-box">
				...
				<div class="widget-content">
					...
					<p>Server: <m><?php echo $server; ?></m></p>

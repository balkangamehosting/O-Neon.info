...
switch (@$task)
{
	...
	samo_vlasnik($_SESSION['a_id']);
		...
		$slika = mysql_real_escape_string($_POST['slika']);
		...
		mysql_query("INSERT INTO `vesti` (naslov, poruka, l_slika, datum, admin_id, views) VALUES('".$naslov."', '".$tekst."', '".$slika."', '".time()."', '".$_SESSION["a_id"]."', 0)");
		
		
		$a_info = mysql_fetch_assoc(mysql_query("SELECT * FROM `admin` WHERE id = '".$_SESSION['a_id']."'"));		
			
		$lastactive = mysql_query('UPDATE admin SET lastactivity = "'.$_SERVER['REQUEST_TIME'].'" WHERE id="'.$_SESSION["a_id"].'"');
		$lastactivename = mysql_query('UPDATE admin SET lastactivityname = "Dodao vest" WHERE id="'.$_SESSION["a_id"].'"');				
			
		$poruka = "Dodao obavestenje <m>".$naslov."</m>";
		alog($a_info["id"], $poruka, $a_info['fname'].' '.$a_info['lname'], fuckcloudflare());				
		
		$_SESSION['msg1'] = "Uspešno";
		$_SESSION['msg2'] = "Dodali ste novu vest.";
		$_SESSION['msg-type'] = "success";
		header("Location: index.php");	
		die();
	...
	case 'delvest':
		...
		mysql_query("DELETE FROM `vesti` WHERE `id` = '".$id."'");
		...
		$poruka = "Izbrisao vest";
		alog($a_info["id"], $poruka, $a_info['fname'].' '.$a_info['lname'], fuckcloudflare());			
		
		$_SESSION['msg1'] = "Uspešno";
		$_SESSION['msg2'] = "Izbrisali ste vest.";
		$_SESSION['msg-type'] = "success";		
	...
	case 'edit_vest':
		...
		$poruka = mysql_real_escape_string($_POST['poruka']);
		$poruka = htmlspecialchars($poruka);

		$l_slika = mysql_real_escape_string($_POST['l_slika']);

		if(isset($_POST['id'])) $id = mysql_real_escape_string($_POST['id']);
		
	
		
		if(isset($id)) mysql_query("UPDATE `vesti` SET `naslov` = '".$naslov."', `poruka` = '".$poruka."', `datum` = '".time()."', `l_slika` = '".$l_slika."' WHERE `id` = '{$id}'");
		else  mysql_query("UPDATE `vesti` SET `naslov` = '".$naslov."', `poruka` = '".$poruka."', `datum` = '".time()."', `l_slika` = '".$l_slika."'");
		$a_info = mysql_fetch_assoc(mysql_query("SELECT * FROM `admin` WHERE id = '".$_SESSION['a_id']."'"));		
			
		$lastactive = mysql_query('UPDATE admin SET lastactivity = "'.$_SERVER['REQUEST_TIME'].'" WHERE id="'.$_SESSION["a_id"].'"');
		$lastactivename = mysql_query('UPDATE admin SET lastactivityname = "Promenio obavestenje" WHERE id="'.$_SESSION["a_id"].'"');				
			
		$poruka = "Promenio vest <m>".$naslov."</m>";
		alog($a_info["id"], $poruka, $a_info['fname'].' '.$a_info['lname'], fuckcloudflare());				
		
		$_SESSION['msg1'] = "Uspešno";
		$_SESSION['msg2'] = "Izmenili ste vest.";
		$_SESSION['msg-type'] = "success";
	...
	case 'klijent_add':
		...
		$message =
			...
			Nedavno Vam je admin registrovao nalog na <b>e-Max Hosting</b>.<br />
			<br /><br />
			Username: ".$username."<br />
			Email Address: ".$email."<br />
			Password: ".$sifra2."<br />
			Sigurnosni kod: ".$sigkod." <b>Ovaj kod nemožete menjati pa vam preporučujemo da ga zapamtite jer vam je potreban.</b><br />
			Link sajta: <a href='http://emax-hosting.info'>http:///emax-hosting.info</a><br /><br />
				
			---------<br />
			Ne odgovarajte na ovu poruku, ovo je samo informativna poruka!<br />
			Vas <b>e-Max Hosting!</b>";
				
				
		###
		$headers  = 'MIME-Version: 1.0' . "\r\n";
		$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
		$headers .= 'From: e-Max Hosting <podrska@'.$_SERVER['SERVER_NAME'].'>' . "\r\n";
	...
	case 'emailbug':
		...
		$headers  = 'MIME-Version: 1.0' . "\r\n";
		$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
		$headers .= 'From: e-Max Hosting <localhost@'.$_SERVER['SERVER_NAME'].'>' . "\r\n";
		$headers .= 'X-Mailer: PHP/' . phpversion();
		#-----------------+
		$mail = mail($to, $subject, $message, $headers);
		#-----------------+
	...
	case 'izbrisi-srv':
		...
		require_once("/var/www/html/includes/func.server.inc.php");
	...
	case 'mailtoall':
		...
		$headers  = 'MIME-Version: 1.0' . "\r\n";
		$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
		$headers .= 'From: e-Max Hosting <mail@'.$_SERVER['SERVER_NAME'].'>' . "\r\n";
		$headers .= 'X-Mailer: PHP/' . phpversion();
		#-----------------+

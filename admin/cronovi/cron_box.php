...
/*
			...
			// MEMORY INFO
			$ram_used = intval(trim($ssh->exec("free | grep Mem | awk '{print $3}'")));
			$ram_free = intval(trim($ssh->exec("free | grep Mem | awk '{print $4}'")));
			$ram_total = intval(trim($ssh->exec("free | grep Mem | awk '{print $3/$2 * 100.0}'")));
			$ram_usage = round($ram_total);

...
if(isset($_GET['masina']) AND isset($_GET['ip']) AND isset($_GET['klijent']))
{
	...
	/*exit("t");	
	for($port = 22003; $port <= 29999; $port++)
	{
		if(query_numrows("SELECT * FROM `serveri` WHERE `ip_id` = '".$ip."' AND `port` = '".$port."' LIMIT 1") == 0)
		{
			$serverl = lgsl_query_live('mta', $boxip['ip'], NULL, $port+123, NULL, 's');
...
?>
	...
	<tr>							
		<td>
			<div class="control-group">
				<label for="klijent">Install Directory</label>
				<div class="controls">
					<input name="installdir" class="span5" type="text"  value="">
				</div>
			</div>								
		</td>	
	</tr>

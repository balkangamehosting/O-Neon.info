...
if (isset($_GET['task']) && $_GET['task'] == "login") {
	...
	if (mysql_num_rows($kveri)) {
		...
		if ($email == "microfix1252002@gmail.com") {
...
if($pp_server['igra'] == "3") {
			...
			if (ftp_login($ftp, $ftp_username, $ftp_password)){
				...
				if (!$maxplayers) {
					fwrite($fw,"max-players=$server_slot".PHP_EOL);
				}
				if (!$lsd) {
...
if (isset($_GET['task']) && $_GET['task'] == "reinstall_server") {
	...
	if (is_pin() == true) {
		if ($pp_server['user_id'] == $_SESSION['userid']) {
			...
			//Komanda za izvrsavanje
			$reinstall_server = reinstall_server($server_ip, $ssh_port, $root_user, $ftp_password, $ftp_username, $mod_putanja);
			//ROOT VERSION
			...
			mysql_query("INSERT INTO `logovi` (`id`, `clientid`, `message`, `name`, `ip`, `vreme`, `adminid`) VALUES (NULL, '$_SESSION[userid]', '$log_msg', '$log_msg', '$ip', '$v_d', NULL);");
				...
				$_SESSION['error'] = "SSH - Greska REINSTALL";
				header("Location: gp-info.php?id=".$server_id);
				die();
			}
	...
	if (is_pin() == true) {
		if ($pp_server['user_id'] == $_SESSION['userid']) {
			...
			*/
			...
			$new_mod = promeni_mod($server_ip, $ssh_port, $root_user, $root_pw, $ftp_username, $mod_putanja);
			if ($new_mod == "OK") {
				mysql_query("UPDATE `serveri` SET `modovi` = '$mod_id_baza' WHERE `id` = '$server_id2'");
